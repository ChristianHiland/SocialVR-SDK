...
namespace SocialSDK {
    ...
    public class AssignmentData {
        ...
        public ScriptType fromScriptType;
        ...
        void Awake() {
            foreach (AssignmentData data in assignmentData) {
                ...
                if (data.fromScript == null) {
                    if (data.fromScriptType == ScriptType.PlayerControl) {
                        fromScript = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerControl>();
                    } else if (data.fromScriptType == ScriptType.UIEventSystem) {
                        fromScript = GameObject.Find("UI EventSystem").GetComponent<EventSystem>();
                    }
                } else {
